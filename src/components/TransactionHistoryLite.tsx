    ...
    <div className="flex flex-col gap-3">
      {symbolHistory.length === 0 ? (
        ...
        symbolHistory.map((t) => {
          ...
          const timeString = new Date(t.timestamp).toLocaleTimeString([], { hour12: false });
          
          return (
            <div key={t.id} className="grid grid-cols-4 items-center text-[13px] font-medium py-1.5 border-b border-border/20 last:border-0 hover:bg-black/5 transition-colors px-2 rounded -mx-2">
              <span className="text-foreground/70 font-semibold">{isBuy ? 'Buy' : 'Sell'}</span>
              ...
              <span className="text-foreground/50 text-right font-mono text-[11px] font-bold">{timeString}</span>
            </div>
          );
        })
      )}
