...
export default function PortfolioSummary() {
	const { balance, totalValue, totalPL, totalPLPercentage, assetStats } =
		usePortfolioStats();
					...
					<Table>
						<TableHeader>
							<TableRow className="bg-secondary hover:bg-secondary border-none h-12">
								<TableHead className="text-xs lg:text-sm font-bold uppercase tracking-widest pl-6 text-white">
									Asset
								</TableHead>
								<TableHead className="text-right text-xs lg:text-sm font-bold uppercase tracking-widest text-white">
									Price
								</TableHead>
								<TableHead className="text-right text-xs lg:text-sm font-bold uppercase tracking-widest pr-6 text-white">
									Value
								</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							<TableRow className="hover:bg-primary/5 transition-colors border-border/50 h-16">
								...
								<TableCell className="text-right text-muted-foreground font-mono font-medium text-sm">
									$1.00
								</TableCell>
							...
							{activeHoldings.map((asset) => (
								<TableRow
									key={asset.symbol}
									className="hover:bg-primary/5 transition-colors border-border/50 h-16 group"
								>
									...
									<TableCell className="text-right pr-6">
										<div className="flex flex-col items-end">
											...
											>
												{asset.pl >= 0 ? "+" : "-"}$
												{Math.abs(asset.pl).toLocaleString(undefined, {
													minimumFractionDigits: 2,
													maximumFractionDigits: 2,
												})}
											</span>
							...
							{activeHoldings.length === 0 && (
								<TableRow>
									<TableCell
										colSpan={3}
										className="text-center py-12 text-muted-foreground font-medium text-sm"
									>
