...
export default function Header() {
  ...
  const { symbols, selectedSymbol, currentPrices } = useSelector((state: RootState) => state.prices);
                    ...
                    filteredSymbols.map(s => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <CoinIcon symbol={s} className="w-6 h-6 border bg-white" />
                          <span className="group-hover/item:text-primary transition-colors">{s?.replace('USDT', '') ?? s}/USDT</span>
                        </div>
                        <span className="text-muted-foreground font-mono group-hover/item:text-foreground transition-colors">
                          {currentPrices[s] ? `$${currentPrices[s].toLocaleString(undefined, { minimumFractionDigits: 2 })}` : ''}
                        </span>
                      </button>
                    ))
                  ) : (
                    <div className="p-6 text-center text-sm font-medium text-muted-foreground italic">No results found</div>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="h-10 lg:h-12 border-l border-border pl-3 lg:pl-6 hidden sm:flex items-center">
            <div className="flex items-center gap-2">
              <span className="text-base lg:text-xl font-bold tracking-tighter font-mono leading-none">
                {selectedSymbol?.replace('USDT', '')}: {currentPrice ? `$${currentPrice.toLocaleString(undefined, { minimumFractionDigits: 2 })}` : '---'}
              </span>
