		...
		executeTrade: (state, action: PayloadAction<Trade>) => {
			...
			if (trade.type === "BUY") {
				if (state.balance >= trade.total) {
					...
					state.holdings[symbol] = Number(
						(currentHolding + trade.amount).toFixed(precision),
					);
					state.costBasis[symbol] = Number(
						(currentCost + trade.total).toFixed(4),
					);
...
export const { executeTrade, resetPortfolio, hydratePortfolio } =
