...
const PriceChart = dynamic(() => import("@/components/PriceChart"), {
	ssr: false,
	loading: () => (
		<div className="w-full h-[400px] bg-muted/20 animate-pulse rounded-xl" />
	),
});

export default function TradePage() {
	const { selectedSymbol, currentPrices } = useSelector(
			...
			<div className="flex-1 flex flex-col relative w-full lg:border-r border-border pb-8 lg:pb-0">
				...
				<div className="px-6 lg:px-12 pt-8 lg:pt-10 pb-4 flex justify-between items-center">
					...
					<div className="text-right">
						<span className="text-xs lg:text-sm text-muted-foreground font-medium block uppercase tracking-wider">
							Available
						</span>
						...
						<div className="text-[10px] lg:text-xs text-muted-foreground font-bold mt-0.5">
							{(cryptoAmount * currentPrice).toLocaleString(undefined, {
				...
				<div className="text-center mt-6 lg:mt-12 space-y-2 lg:space-y-4 z-10 px-6">
					<h2 className="text-xl lg:text-3xl font-medium tracking-widest text-foreground opacity-80">
						{rawSymbol}
					</h2>
					...
					<div
						className={`text-sm lg:text-lg font-bold px-3 py-1 bg-opacity-10 rounded-full inline-block ${isPositive ? "text-emerald-500 bg-emerald-500/10" : "text-rose-500 bg-rose-500/10"}`}
					>
						PnL: {isPositive ? "+" : ""}
