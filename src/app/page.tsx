...
export default function DashboardPage() {
  ...
  const { symbols, marketData, currentPrices } = useSelector((state: RootState) => state.prices);
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-secondary hover:bg-secondary border-none h-12">
                  <TableHead className="text-xs lg:text-sm font-black uppercase tracking-widest pl-6 text-white">Asset</TableHead>
                  <TableHead className="text-right text-xs lg:text-sm font-black uppercase tracking-widest text-white">Last Price</TableHead>
                  <TableHead className="text-right text-xs lg:text-sm font-black uppercase tracking-widest pr-6 text-white">24h Change</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {symbols.map((symbol) => {
                    ...
                    >
                      <TableCell className="pl-6">
                        <div className="flex items-center gap-4">
                          <CoinIcon symbol={symbol} className="w-10 h-10 shadow-sm transition-transform group-hover:scale-110" />
                          <div className="flex flex-col">
                            <span className="font-bold text-base tracking-tight">{symbol.replace('USDT', '')}</span>
